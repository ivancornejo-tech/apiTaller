...
router.get('/spic/dependencias', (req, res) => {
    ...
    MongoClient.connect(dbConf.url, dbConf.client_options).then(client => {
        ...
        spic.aggregate([{
            $group: {
                _id: {
                    nombre: "$institucionDependencia.nombre",
                    siglas: "$institucionDependencia.siglas",
                    clave: "$institucionDependencia.clave"
                }
            }
...
router.post('/spic', (req, res) => {
    ...
    if (typeof query !== 'undefined') {
        ...
        if (query.hasOwnProperty('institucionDependencia') && query['institucionDependencia'].length > 0) {
            _query['institucionDependencia.nombre'] = {
                $regex: query['institucionDependencia'],
                $options: 'i'
            };
        }
    }
    ...
    MongoClient.connect(dbConf.url, dbConf.client_options).then(client => {
        ...
        let cursor = spic.find(_query).skip(skip).limit(pageSize).collation({
            locale: "en",
            strength: 1
        });
...
router.put('/spic', (req, res) => {
    ...
    Spic.findByIdAndUpdate(id, spic, {
        new: true
    }).then(sp => {
        res.json(sp);
        mongoose.disconnect();
